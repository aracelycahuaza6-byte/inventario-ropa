...
export default function Home() {
  ...
  async function loadItems() {
    setLoading(true);
    const { data, error } = await supabase.from("prendas").select("*").order("created_at", { ascending: false });
  ...
  async function auth() {
    ...
    else setAuthMessage(authMode === "signup" ? "Cuenta creada. Revisa tu correo si Supabase solicita confirmación." : "");
  }
  ...
  async function toggleSold(item: Prenda) {
    const next = item.estado === "disponible" ? "vendido" : "disponible";
    const { error } = await supabase.from("prendas").update({
      estado: next,
      sold_at: next === "vendido" ? new Date().toISOString() : null
    }).eq("id", item.id);
    if (error) setMessage(error.message);
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter(i => {
      ...
      const matchesSearch = !q || [i.code, i.nombre, i.marca, i.categoria, i.talla]
  ...
  const stats = useMemo(() => {
    ...
    const compra = items.reduce((s, i) => s + Number(i.precio_compra || 0), 0);
    const ventas = vendidos.reduce((s, i) => s + Number(i.precio_venta || 0), 0);
    const ganancia = vendidos.reduce((s, i) => s + Number(i.precio_venta || 0) - Number(i.precio_compra || 0), 0);
    return { total: items.length, disponibles: disponibles.length, vendidos: vendidos.length, compra, ventas, ganancia };
  }, [items]);
    ...
    return (
      <main className="auth-page">
        <section className="auth-card">
          ...
          <p className="muted">Tu inventario queda guardado en Supabase y puedes acceder desde celular o computadora.</p>
          <input placeholder="Correo" type="email" value={email} onChange={e => setEmail(e.target.value)} />
          <input placeholder="Contraseña" type="password" value={password} onChange={e => setPassword(e.target.value)} />
          <button className="primary" onClick={auth}>{authMode === "login" ? "Entrar" : "Crear cuenta"}</button>
        ...
        <div className="grid">
          <section className="card">
            ...
            <form onSubmit={addItem} className="form">
              ...
              <div className="two"><input placeholder="Marca" value={form.marca} onChange={e => setForm({...form, marca:e.target.value})} /><input placeholder="Talla" value={form.talla} onChange={e => setForm({...form, talla:e.target.value})} /></div>
              <input placeholder="Categoría (polo, jean, casaca...)" value={form.categoria} onChange={e => setForm({...form, categoria:e.target.value})} />
              <div className="two"><input type="number" min="0" step="0.01" placeholder="Precio compra" value={form.precio_compra} onChange={e => setForm({...form, precio_compra:e.target.value})} /><input type="number" min="0" step="0.01" placeholder="Precio venta" value={form.precio_venta} onChange={e => setForm({...form, precio_venta:e.target.value})} /></div>
          ...
          <section className="card">
            ...
            <div className="table-wrap">
              <table>
                <thead><tr><th>Foto</th><th>Código</th><th>Prenda</th><th>Compra</th><th>Venta</th><th>Ganancia</th><th>Estado</th><th></th></tr></thead>
                <tbody>
                  {filtered.map(i => (
                    <tr key={i.id}>
                      <td>{i.foto_url ? <img className="thumb" src={i.foto_url} alt={i.nombre} /> : <span className="no-photo">—</span>}</td>
                      <td><b>{i.code}</b></td>
                      ...
                      <td><button className={`status ${i.estado}`} onClick={() => toggleSold(i)}>{i.estado === "disponible" ? "🟢 Disponible" : "🔴 Vendido"}</button></td>
                      <td><button className="danger" onClick={() => removeItem(i)}>Eliminar</button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
